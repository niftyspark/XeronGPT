...
export default function App() {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const audioBase64 = await generateAudio(fullContent);
      if (audioBase64 && audioRef.current) {
        const audioSrc = `data:audio/flac;base64,${audioBase64}`;
      ...
      <div className="flex h-screen items-center justify-center bg-[#111] text-zinc-100 font-sans relative overflow-hidden">
        ...
        <div className="text-center max-w-md p-8 bg-black/40 backdrop-blur-xl rounded-3xl shadow-xl border border-white/5 z-10">
          ...
          <p className="text-zinc-400 mb-8">
            Sign in to start chatting, save your history, and access advanced AI
  ...
  return (
    <div className="flex h-screen bg-[#111] text-zinc-100 font-sans overflow-hidden relative">
      ...
      {/* Sidebar */}
      <div
        className={`${sidebarOpen ? "w-[260px]" : "w-0"} transition-all duration-300 flex-shrink-0 bg-black/40 backdrop-blur-xl border-r border-white/5 flex flex-col overflow-hidden z-20`}
      >
          ...
          {filteredConversations.map((convo) => (
            ...
            >
              <div className="flex items-center gap-2 overflow-hidden">
                <MessageSquare
                  size={16}
                  className={`flex-shrink-0 ${currentConversationId === convo.id ? "text-zinc-200" : "text-zinc-500 group-hover:text-zinc-400"}`}
                />
                <span
                  className={`truncate ${currentConversationId === convo.id ? "text-zinc-100" : "text-zinc-400 group-hover:text-zinc-300"}`}
                >
                  {convo.title}
                </span>
              </div>
      ...
      {/* Main Content */}
      <div
        className="flex-1 flex flex-col h-full relative min-w-0 z-10 bg-cover bg-center"
        style={{
          backgroundImage:
      ...
      >
        ...
        {/* Header */}
        <header className="h-14 flex items-center justify-between px-4 z-10 bg-black/20 backdrop-blur-md border-b border-white/10">
          <div className="flex items-center gap-3">
            <button
              onClick={() => setSidebarOpen(!sidebarOpen)}
              className="p-2 -ml-2 rounded-lg hover:bg-zinc-800 text-zinc-400 hover:text-lime-400 transition-colors"
            >
              <Menu size={20} />
            </button>
          ...
          {currentConversationId && (
            <button
              onClick={(e) =>
                handleDeleteConversation(e, currentConversationId)
              }
        ...
        <div className="flex-1 flex overflow-hidden z-10 relative">
          {/* 3D Character Area */}
          <div className="w-1/3 h-full relative hidden lg:block pointer-events-none">
          ...
          <div className="flex-1 flex flex-col h-full relative">
              ...
              <div className="max-w-3xl mx-auto flex flex-col gap-6 pb-32">
                {messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-[60vh] text-center px-4">
                    <div className="w-20 h-20 bg-black/40 backdrop-blur-xl rounded-full flex items-center justify-center mb-6 shadow-2xl border border-white/10 relative overflow-hidden">
                      <div className="absolute inset-0 bg-lime-400/20 blur-xl rounded-full"></div>
                      <Bot size={40} className="text-lime-400 relative z-10" />
                    </div>
                    <h1 className="text-3xl font-semibold mb-3 text-white drop-shadow-lg">
                      How can I help you today?
                    </h1>
                    <p className="text-zinc-300 max-w-md text-base drop-shadow-md">
                      Experience the power of multiple AI models. Upload files,
                      search the web, and get intelligent responses.
                    </p>
                  </div>
                ) : (
                  messages.map((msg) => (
                    ...
                    >
                      <div
                        className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-1 backdrop-blur-md border border-white/10 shadow-lg ${msg.role === "user" ? "bg-black/40 text-zinc-300" : "bg-lime-400/20 text-lime-400"}`}
                      >
                        {msg.role === "user" ? (
                          <User size={16} />
                        ) : (
                      ...
                      >
                        ...
                        {msg.attachments && msg.attachments.length > 0 && (
                          <div className="flex flex-wrap gap-2 mb-1">
                            {msg.attachments.map((att) => (
                              <div
                                key={att.id}
                                className="flex items-center gap-2 bg-black/40 backdrop-blur-md p-2 rounded-xl border border-white/10 max-w-xs shadow-lg"
                              >
                                {att.type === "image" ? (
                                  <img
                                    src={att.data}
                                  ...
                                  <div className="w-12 h-12 bg-black/50 rounded-lg flex items-center justify-center border border-white/5">
                                    <FileText
                                      size={20}
                                ...
                                <div className="text-sm truncate pr-2 font-medium text-zinc-200">
                                  {att.name}
                                </div>
                              </div>
                            ))}
                        ...
                        {msg.content && (
                          <div
                            className={`px-5 py-3.5 rounded-3xl backdrop-blur-md border border-white/10 ${msg.role === "user" ? "bg-black/40 text-zinc-100" : "bg-black/60 text-zinc-100 shadow-xl"}`}
                          >
                            {msg.role === "user" ? (
                              ...
                              <div className="prose prose-invert prose-zinc max-w-none prose-p:leading-relaxed prose-pre:bg-black/50 prose-pre:border prose-pre:border-white/10 prose-pre:rounded-xl">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 via-black/50 to-transparent">
              <div className="max-w-3xl mx-auto relative">
                <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl p-3 shadow-2xl relative">
                  {/* Attachments Preview */}
                  {attachments.length > 0 && (
                    <div className="absolute bottom-full left-0 mb-3 flex flex-wrap gap-2 p-2 bg-black/40 backdrop-blur-xl rounded-2xl border border-white/10 w-full shadow-2xl">
                      {attachments.map((att) => (
                        <div
                          key={att.id}
                          className="relative group flex items-center gap-2 bg-black/50 p-2 rounded-xl border border-white/10 max-w-[200px]"
                        >
                          {att.type === "image" ? (
                            <img
                              src={att.data}
                          ...
                          <div className="text-xs truncate flex-1 font-medium text-zinc-200">
                            {att.name}
                          </div>
                          <button
                            onClick={() => removeAttachment(att.id)}
                            className="absolute -top-2 -right-2 w-6 h-6 bg-zinc-800 hover:bg-red-500 border border-white/10 rounded-full flex items-center justify-center text-white transition-colors opacity-0 group-hover:opacity-100 shadow-lg"
                          >
                            <X size={14} />
                          </button>
                        </div>
                  ...
                  <div className="relative flex flex-col bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl focus-within:border-lime-500/50 transition-all">
                    <textarea
                      ref={textareaRef}
                      value={input}
                    ...
                    <div className="flex items-center justify-between px-3 pb-3">
                      <div className="flex items-center gap-1">
                        ...
                        <button
                          ...
                          className="p-2 rounded-full text-zinc-400 hover:text-lime-400 hover:bg-zinc-800/50 transition-colors"
                          title="Attach file"
                        >
                          <Paperclip size={20} />
                        </button>
                        <button
                          ...
                          className={`p-2 rounded-full transition-colors flex items-center gap-2 ${webSearch ? "text-lime-400 bg-lime-400/10" : "text-zinc-400 hover:text-lime-400 hover:bg-zinc-800/50"}`}
                          title="Web Search"
                        >
                          <Globe size={20} />
                          {webSearch && (
                        ...
                        <button
                          ...
                          className={`p-2 rounded-full transition-colors flex items-center gap-2 ${liveBrowser ? "text-blue-400 bg-blue-400/10" : "text-zinc-400 hover:text-lime-400 hover:bg-zinc-800/50"}`}
                  ...
                  <div className="text-center mt-3">
                    <span className="text-xs text-zinc-400 drop-shadow-md">
                      AI can make mistakes. Consider verifying important
                      information.
                    </span>
                  </div>
                </div>
