...
import { Send, Paperclip, Globe, Plus, MessageSquare, Menu, X, ChevronDown, User, Bot, FileText, Loader2, LogOut, Search, Trash2, Compass } from 'lucide-react';
...
export default function App() {
  ...
  const handleSend = async () => {
    ...
    if (!convoId) {
      const title = input.trim().substring(0, 40) || "New Conversation";
  ...
  if (!user) {
    return (
      <div className="flex h-screen items-center justify-center bg-[#212121] text-zinc-100 font-sans">
        <div className="text-center max-w-md p-8 bg-[#2f2f2f] rounded-3xl shadow-xl border border-zinc-700">
  ...
  return (
    <div className="flex h-screen bg-[#212121] text-zinc-100 font-sans overflow-hidden">
      {/* Sidebar */}
      <div className={`${sidebarOpen ? 'w-[260px]' : 'w-0'} transition-all duration-300 flex-shrink-0 bg-[#171717] flex flex-col overflow-hidden`}>
        <div className="p-3 pb-0">
          <button 
            onClick={startNewChat}
            className="flex items-center gap-2 w-full p-2.5 rounded-lg hover:bg-zinc-800 transition-colors text-sm font-medium"
          >
      ...
      <div className="flex-1 flex flex-col h-full relative min-w-0">
        ...
        <header className="h-14 flex items-center justify-between px-4 z-10">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 -ml-2 rounded-lg hover:bg-zinc-800 text-zinc-400 hover:text-zinc-100 transition-colors">
              <Menu size={20} />
            </button>
          ...
          <div className="max-w-3xl mx-auto flex flex-col gap-6 pb-32">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-[60vh] text-center px-4">
                <div className="w-16 h-16 bg-zinc-800 rounded-full flex items-center justify-center mb-6 shadow-sm border border-zinc-700">
                  <Bot size={32} className="text-zinc-300" />
                </div>
            ...
            <div className="relative flex flex-col bg-[#2f2f2f] border border-zinc-700 rounded-3xl shadow-lg focus-within:border-zinc-500 transition-all">
              ...
              <div className="flex items-center justify-between px-3 pb-3">
                ...
                <button
                  ...
                  className="p-2 rounded-full bg-white text-black hover:bg-zinc-200 disabled:opacity-50 disabled:hover:bg-white transition-colors flex items-center justify-center w-10 h-10"
                >
